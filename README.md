...
## 10. Customer Journey & Business Insights (added modules)

The dashboard now has five tabs — 🏠 Overview, ⚡ Website Performance (the
original dashboard above, unchanged), 🛒 Customer Journey, 📈 Business
Insights, 📊 History, and ⚙ Settings — switched client-side with no page
reload. Each new tab is its own independent module; deleting any one of
them (its `.js` + `.css` + backend `.py` files) removes only that tab and
does not affect PageSpeed monitoring, Google Sheets, or email reports.

```
settings_data.py     Reads config.py, writes dashboard/data/settings.json
journey_config.py    Customer Journey settings (URLs, selectors, retry timing)
journey.py            Playwright bot: Homepage → Collection → Product →
                       Cart → Checkout, stopping before payment/OTP/order
journey_email.py     Sends the "URGENT — Customer Journey Failure" alert
journey_data.py      Reads data/journey_runs.json, writes dashboard/data/journey.json
insights_config.py   Business Health Score weights and thresholds
insights_data.py     Combines summary.json + journey.json into dashboard/data/insights.json

dashboard/js/  business.js   Lighthouse-issue -> plain-English dictionary (shared)
               nav.js        Tab switching only
               overview.js / settings.js / history.js / journey.js / insights.js
dashboard/css/ navigation.css, overview.css, business.css, settings.css,
               history.css, journey.css, insights.css

.github/workflows/
  journey-monitor.yml   Runs journey.py every 15 minutes, installs Playwright's
                         Chromium browser, commits journey + insights data,
                         deploys the same GitHub Pages site as monitor.yml
```

**Additional setup for these modules:**
- No new repository secrets are required — Customer Journey and Business
  Insights reuse the same `SMTP_*` secrets already used for report emails,
  and `GOOGLE_PAGESPEED_API_KEY` / `GOOGLE_SHEET_ID` are inherited by
  `journey-monitor.yml` even though Journey monitoring itself doesn't call
  either API (kept for parity with `monitor.yml`'s secret list; harmless).
- Local testing: `pip install -r requirements.txt && python -m playwright
  install chromium`, then `python journey.py`.
- **Please verify the CSS selectors in `journey.py`** (`button[name='add']`,
  `.price`, `h1`, the cart/checkout link selectors) against goodmonk.in's
  actual live theme before relying on alerts — they're solid defaults for a
  standard Shopify theme but themes vary, and a wrong selector would show
  as a false "Add To Cart Button Visible" failure rather than a real one.
- The two workflows (`monitor.yml`, hourly/daily; `journey-monitor.yml`,
  every 15 minutes) share the `pages` concurrency group, so GitHub queues
  them instead of running them simultaneously — there's no git-push race
  between them even though both can commit to `dashboard/data/`.
- Failure screenshots/HTML are only captured after a **second consecutive**
  Customer Journey failure (a single failure is treated as transient and
  retried silently), and only the 5 most recent capture folders are kept,
  so this can't grow the repo unbounded.
