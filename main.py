...
def main() -> int:
    ...
    # Additive, independent of the existing dashboard data build above —
    # powers the new Settings tab only. Never allowed to fail the run.
    try:
        settings_data.build_settings()
    except Exception as e:  # noqa: BLE001
        log.error("Failed to rebuild settings data: %s", e)

    # Additive — powers the Business Insights tab. Reads summary.json /
    # journey.json read-only; never allowed to fail the run.
    try:
